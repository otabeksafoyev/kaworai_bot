...
GENRE_ALIASES = {
    "Sci-Fi": "SciFi", "Slice of Life": "SliceOfLife",
    "sci-fi": "SciFi", "slice of life": "SliceOfLife",
    "action": "Action", "adventure": "Adventure",
    "comedy": "Comedy", "drama": "Drama",
    "fantasy": "Fantasy", "horror": "Horror",
    "mystery": "Mystery", "romance": "Romance",
    "scifi": "SciFi", "sliceoflife": "SliceOfLife",
    "sports": "Sports", "sport": "Sports",
    "supernatural": "Supernatural", "thriller": "Thriller",
    "mecha": "Mecha", "magic": "Magic",
    "school": "School", "shounen": "Shounen",
    "shoujo": "Shoujo", "isekai": "Isekai",
...
def normalize_genre(g: str) -> str:
    g = g.strip()
    if g in GENRE_ALIASES:         return GENRE_ALIASES[g]
    if g.lower() in GENRE_ALIASES: return GENRE_ALIASES[g.lower()]
    for key in GENRES:
        if key.lower() == g.lower(): return key
    return g


def parse_genres(raw_genres) -> list:
    if raw_genres is None:          return []
    if isinstance(raw_genres, list): return raw_genres
...
async def _is_pro(user_id: int) -> bool:
    async with AsyncSessionLocal() as session:
        ...
        if not user or not user.is_pro: return False
        if user.pro_until and user.pro_until < datetime.utcnow():
            user.is_pro = False; user.pro_until = None
...
def anime_list_keyboard(
    animes: list, genre_key: str, from_page: int, page: int = 0
