...
def _build_episode_keyboard(
    ...
    if page > 0:
        nav_row.append(
            InlineKeyboardButton(text="◄ Oldingi", callback_data=f"eppage_{anime_id}_{current_ep}_{page - 1}")
        )
    else:
        nav_row.append(InlineKeyboardButton(text="◄ Oldingi", callback_data=f"epnav_{anime_id}_{current_ep}_prev"))
    nav_row.append(InlineKeyboardButton(text=f"{page + 1} / {total_pages}", callback_data="ep_noop"))
    if page < total_pages - 1:
        nav_row.append(
            InlineKeyboardButton(text="Keyingi ►", callback_data=f"eppage_{anime_id}_{current_ep}_{page + 1}")
        )
    else:
        nav_row.append(InlineKeyboardButton(text="Keyingi ►", callback_data=f"epnav_{anime_id}_{current_ep}_next"))
    builder.row(*nav_row)
...
async def _show_anime_card_inline(message: types.Message, anime_id: int, user_id: int):
    ...
    kb_rows.append(
        [
            ...
            InlineKeyboardButton(text="🔍 anime qidirsh", switch_inline_query_current_chat=""),
        ]
    )

    kb_rows.append([InlineKeyboardButton(text="🏠 menu", callback_data="main_menu")])
...
async def _show_anime_card(message: types.Message, anime_id: int, user_id: int, from_inline: bool = False):
    ...
    if anime.is_pro_locked and not is_pro:
        ...
        kb_rows.append(
            [InlineKeyboardButton(text="▶️ 1-qismdan tomosha qilish", callback_data=f"watch_start_{anime_id}")]
        )
