...
async def _show_anime_card_inline(message: types.Message, anime_id: int, user_id: int):
    ...
    async with AsyncSessionLocal() as session:
        ...
        first_ep = (await session.execute(
            select(Series)
    ...
    kb_rows.append([
        InlineKeyboardButton(
            text=f"♥ obuna bo'lganlar ({sub_count})",
...
async def episode_select(call: types.CallbackQuery):
    ...
    try:
        ...
        try:
            await call.message.answer_video(
                video=ep_obj.file_id, caption=caption,
                reply_markup=kb, parse_mode="HTML",
                protect_content=not is_pro,
            )
...
async def episode_navigate(call: types.CallbackQuery):
    ...
    try:
        ...
        try:
            await call.message.answer_video(
                video=new_ep_obj.file_id, caption=caption,
                reply_markup=kb, parse_mode="HTML",
                protect_content=not is_pro,
            )
        except Exception as e2:
...
@user_router.callback_query(
    ...
    & ~F.data.startswith("rate_set_")
    & ~F.data.startswith("rate_cancel")
)
